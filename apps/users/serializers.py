...
class UserProfileSerializer(serializers.ModelSerializer):
	...
	code = serializers.CharField(help_text="验证码",label="手机收到的验证码",write_only=True,allow_blank=False,max_length=4,min_length=4,required=True)

	#已经要填写，不允许为空，不能注册重复
	username = serializers.CharField(help_text="用户注册手机号",label="注册手机号",required=True,allow_blank=False, validators=[UniqueValidator(queryset=UserProfile.objects.all(),message="注册的手机号或者用户名不能重复")])

	#用户输入的密码
	password = serializers.CharField(help_text="用户登录密码",write_only=True,label="注册密码",required=True,allow_blank=False,min_length=6,style={"input_type":"password"})
